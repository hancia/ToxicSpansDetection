...
if __name__ == '__main__':
    ...
    # df = pd.read_csv('filled/spans-pred.txt', header=None, names=['spans'], sep='\t')
    print(df.head())
    df['spans'] = df['spans'].apply(fill_holes_in_row)
    df['spans'] = df['spans'].apply(remove_ones_in_row)
    print(df.head())
    df.to_csv('filled/spans-pred-filled-cleaned.txt', header=False, sep='\t', quoting=csv.QUOTE_NONE, escapechar='\n')
