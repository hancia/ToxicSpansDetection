...
def plot_SP_density_by_class(ax, spcorr, yhat, linestyle='-', label=None) :
    sprho = np.array([x[0] for x in spcorr])
    sppval = np.array([x[1] for x in spcorr])
...
def plot_histogram_by_class(ax, values, yhat, bins=40, hist_lims=None,label=None) :
    ...
    if unique_y is not None and len(unique_y) < 4:
        for y in unique_y :
            rho = values[yhat == y]
            ax.hist(rho, bins=bins, range=hist_lims, alpha=0.6, linewidth=0.5, edgecolor='k', weights=np.ones(len(rho))/len(rho), label=label)
    else :
        ax.hist(values, bins=bins, range=hist_lims, alpha=0.6, linewidth=0.5, edgecolor='k', weights=np.ones(len(values))/len(values),label=label)
...
def plot_boxplot(ax, Y_vals, classes):
    
    df = {'y':[],'class':[]}
    for y,class_ in zip(Y_vals,classes):
        df['y'].extend(y)
        df['class'].extend([class_]*len(y))

    df = pd.DataFrame({'y' : df['y'], 'class' : df['class']})

    ax = sns.boxplot(x="class", y="y", data=df)

def plot_matrix(ax,matrix, rows, columns):
    
    df_cm = pd.DataFrame(matrix, index = [i for i in rows],columns = [i for i in columns])
    ax = sns.heatmap(df_cm, annot=True, ax=ax,fmt='.3g',linewidths=1,cmap=sns.color_palette("Blues"))
    return ax

def plot_matrix1(ax,matrix, rows, columns,cmap):
    
    df_cm = pd.DataFrame(matrix, index = [i for i in rows],columns = [i for i in columns])
    ax = sns.heatmap(df_cm, ax=ax,fmt='.3g',linewidths=1,cmap=cmap)
    return ax

def plot_attention(ax,matrix, annotate):
    
    ax = sns.heatmap(matrix, annot=annotate, vmin=0, vmax=0.5,ax=ax,linewidths=1,fmt="",cmap=sns.light_palette("navy"))
    return ax
